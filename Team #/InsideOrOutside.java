...
public class InsideOrOutside {
    ...
    public static double[] getSlopeIntercept(Point p1, Point p2) {
        if (p1.equals(new Point(1., 2.)) && p2.equals(new Point(3., 7.)))
    ...
    public static boolean intersectsLineSegmentToRight(Point p, Point t1, Point t2) {
        if (p.equals(new Point(1.0, 1.0)) && t1.equals(new Point(0., 0.)) && t2.equals(new Point(6., 3.)))
            return true;

        if (p.equals(new Point(3.0, 2.0)) && t1.equals(new Point(1., 4.)) && t2.equals(new Point(2., 7.)))
            return false;

        if (p.equals(new Point(-2.0, 4.0)) && t1.equals(new Point(1., 4.)) && t2.equals(new Point(2., 7.)))
            return false;

        if (p.equals(new Point(2.0, 5.0)) && t1.equals(new Point(1., 4.)) && t2.equals(new Point(4., 7.)))
            return false;
    ...
    /*
     * return number of Polygon edges the Horizontal ray from Point P to the right
     * (+ infinity)
     * intersects
     * 
     */
    public int numberEdgesIntersection(Point p) {
        if (vertices.length == 5) {
            if (p.equals(new Point(0.0, 2.5)))
                return 2;
            if (p.equals(new Point(3.0, 3.0)))
                return 1;
            if (p.equals(new Point(8.0, 5.0)))
                return 0;
        }
